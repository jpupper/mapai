...
ssh -p !VPS_PORT! !VPS_USER!@!VPS_HOST! "mkdir -p mapai && cd mapai && (test -d .git || (git init && git remote add origin !REPO_URL!)) && git remote set-url origin !REPO_URL! && echo 'Bajando cambios al VPS...' && (git fetch origin main || git fetch origin master) && (git checkout -B main origin/main || git checkout -B master origin/master) && (grep -q '^GITHUB_REPO=' .env 2>/dev/null && sed -i 's|^GITHUB_REPO=.*|GITHUB_REPO=jpupper/mapai|' .env || echo 'GITHUB_REPO=jpupper/mapai' >> .env) && echo 'Corriendo el deploy de backend...' && bash deployscripts/server_update.sh"
